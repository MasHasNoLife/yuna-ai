...
plugin_info = {
    ...
    "authentication_token_path": os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "vts_token.txt")
}
...
async def main():
    ...
    try:
        ...
        if not hotkeys:
            print("No hotkeys found! You need to set them up in VTube Studio first (the clapperboard 🎬 icon).")
        else:
