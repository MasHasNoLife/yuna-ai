...
plugin_info = {
    ...
    "authentication_token_path": os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "vts_token.txt"
    ),
}
...
async def main():
    ...
    for p in default_params:
        print(
            f"{p['name']:<30} {p['min']:>8.1f} {p['max']:>8.1f} {p['defaultValue']:>8.1f} {p['value']:>8.2f}"
        )
    ...
    for _ in range(60):
        req = vts.vts_request.requestSetParameterValue(
            parameter="ParamAngleX", value=15.0, weight=1, face_found=True
        )
    ...
    for _ in range(30):
        req = vts.vts_request.requestSetParameterValue(
            parameter="ParamAngleX", value=0.0, weight=1, face_found=True
        )
        await vts.request(req)
        await asyncio.sleep(0.033)
