...
from yuna_prompt import SYSTEM_PROMPT, TEXT_REACTION_PROMPT
import memory
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_text_script.py \"<your text prompt>\"")
        sys.exit(1)
    ...
    prompt = f"{context_prompt}Topic/Scenario:\n\n{text_prompt}"
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT + "\n\n" + TEXT_REACTION_PROMPT},
        {"role": "user", "content": prompt}
    ]

    print(f"[LLM] Generating high-fidelity script based on text prompt...\n")
    ...
    out_file = os.path.join(OUTPUT_DIR, "text_reaction.txt")
    
    counter = 1
    while os.path.exists(out_file):
        out_file = os.path.join(OUTPUT_DIR, f"text_reaction{counter}.txt")
        counter += 1
