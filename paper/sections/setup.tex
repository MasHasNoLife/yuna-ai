...
\textbf{single-hop} (one fact, often verbatim), and \textbf{adversarial}
(unanswerable, so the correct response is to decline). We use the officially released benchmark
...
\begin{itemize}
  ...
  attribution. This measures \emph{precision} (of what is stored, how much is
  correct) directly; \emph{recall} (whether the extractor missed facts) is proxied by
  end-to-end QA accuracy (\S\ref{sec:results-strategies}), since a missed fact
  surfaces as a wrong answer. \S\ref{sec:disc-threats} discusses the limitation.
...
repository, with step-by-step commands to reproduce every table and figure.
