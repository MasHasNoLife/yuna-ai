...
reference application (Yuna). The design goal is a \emph{persistent} conversational
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{fig2_architecture.pdf}
\caption{The three-stage per-turn pipeline. Recall and generation are on the
interactive path; extraction is dispatched after the reply streams, so it never
delays time-to-first-token.}
\label{fig:architecture}
\end{figure}

Each turn proceeds in three stages (Figure~\ref{fig:architecture}):

\begin{enumerate}
  \item \textbf{Recall.} The user message (plus the previous one, for pronoun context) is
  ...
  date/time preamble.
  \item \textbf{Generation.} The chat model produces the reply, streamed to the user.
  \item \textbf{Extraction.} \emph{After} the reply is sent, a background call to the extractor
  model converts the exchange into typed memory operations and applies them to
  the store (\S\ref{sec:method-async}).
\end{enumerate}

Memory is a single vector store (ChromaDB) with typed metadata; there is no
separate symbolic database.

\subsection{The typed memory protocol}
...
\begin{itemize}
  \item \textbf{[FACT]}---a durable truth about the user or world (``Mas's favourite dish is
  carbonara'').
  \item \textbf{[EVENT]}---a dated happening (``Mas started a new job'').
  \item \textbf{[SELF]}---a durable fact the agent revealed about \emph{itself}, routed to a
  ...
  \item \textbf{[UPDATE]} old $\rightarrow$ new---a correction to an existing fact.
...
writes an EVENT (``ran a charity race \emph{(on 20 May 2023)}''), and each memory is
...
result in \S\ref{sec:results-temporal}: the answerer reads a resolved date
directly instead of attempting date arithmetic over a long transcript.
...
previous user message, so pronoun-heavy follow-ups (``no, the \emph{one I told you
about}'') still retrieve the right memory.

\textbf{Age-aware results.} Retrieved EVENT and session memories are annotated with a
human-readable age (``(3 days ago) \ldots'') derived from their stored timestamp, giving
...
\subsection{Asynchronous, non-blocking design}
...
\begin{itemize}
  ...
  (capped at the most recent 60k characters, $\approx$15k tokens).
...
Holding everything else fixed, these isolate the contribution of \emph{extraction}
(agentic vs.\ raw\_rag) and of \emph{retrieval} (both vs.\ full\_history and none).
