...
\section{What the results do and do not claim}
...
1. \textbf{Temporal reasoning.} Distilling and *dating* memories at write time beats
   date arithmetic over raw text (§5.2).
2. \textbf{Length-invariance.} Retrieval-based memory is indifferent to conversation
...
3. \textbf{Efficiency.} Competitive accuracy at ~67× less context, within a 4k-token
...
\section{Where agentic memory loses, and why}
...
\textbf{consolidation pass} (§6.4) that periodically merges related memories and
...
\begin{itemize}
  ...
  \item \textbf{MSC and persona consistency.} A second benchmark measuring long-horizon
  persona stability, not just factual recall.
  \item \textbf{Extraction precision.} Completing the gold-annotation evaluation (§4.3) to
