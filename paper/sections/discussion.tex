...
We do \textbf{not} claim agentic memory is universally superior at recall. On our
main benchmark, full-context stuffing achieves the highest overall judged
accuracy (\S\ref{sec:results-strategies}), because when the answer to a question
sits verbatim inside the context window, providing the raw text beats providing a
distilled summary of it. Our claims are narrower and, we argue, more consequential
for real deployment:
...
\begin{enumerate}
  ...
  outgrows the window (\S\ref{sec:results-length}). full\_history's overall win is
  contingent on the answer surviving truncation; agentic memory has no such
  dependency.
...
\begin{itemize}
  ...
  \item \textbf{A second truncation-regime benchmark.} Generalising length-invariance
  requires a dataset whose conversations exceed the context window; constructing
  or identifying one is a contribution in itself.
